...
export type ScoreTokenRiskParams = {
  evaluation: 'initial' | 'recheck';
  baselineSnapshot?: TokenBalanceSnapshot;
};

export type ScoreTokenRiskResult = {
  score: RiskScore;
...
export async function scoreTokenRisk(
  ...
  evt: TokenEvent,
  config: AppConfig,
  params: ScoreTokenRiskParams
): Promise<ScoreTokenRiskResult> {
  ...
  let snapshot: TokenBalanceSnapshot | undefined;
  if (config.sellDetectionEnabled || config.devGraphEnabled) {
    const seeds = [evt.deployer, token.mintAuthority, token.freezeAuthority].filter(Boolean) as string[];
