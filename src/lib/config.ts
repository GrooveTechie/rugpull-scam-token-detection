...
export type AppConfig = {
  ...
  // Monitoring / recheck
  monitorRecheckMinutes: number;
  monitorMaxTrackedTokens: number;
  monitorTtlMinutes: number;
  monitorRpcBudgetPerToken: number;

  // Alert dedupe
  alertDedupeTtlMinutes: number;
  alertResendScoreDelta: number;

  // Dev graph
  devGraphEnabled: boolean;
  devGraphLookbackMinutes: number;
  devGraphMaxWallets: number;
  devGraphDepth: number;

  // Sell detection
  sellDetectionEnabled: boolean;
  sellLookbackMinutes: number;
  sellMinTokenDropPct: number;
  sellMinSupplyPct: number;
  proxyWindowMinutes: number;
  proxyMaxHops: number;
  sellDevWeight: number;
  sellProxyWeight: number;
  sellMovementNoSellWeight: number;
};
